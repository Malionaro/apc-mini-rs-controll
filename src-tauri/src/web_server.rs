...
pub fn start_web_server(state: Arc<MidiState>) {
    ...
    thread::spawn(move || {
        ...
        let listener = match TcpListener::bind(&address) {
            Ok(l) => {
                crate::midi::add_log(&state_clone, format!("Web Companion gestartet auf http://localhost:{}", port));
                l
            }
            Err(e) => {
                crate::midi::add_log(&state_clone, format!("Fehler beim Starten des Web Companions: {}", e));
                return;
            }
        };
                ...
                if note_val != 255 {
                    ...
                    thread::spawn(move || {
                        ...
                        if let Some(mapping) = mapping_opt {
                            crate::midi::add_log(&state_clone, format!("Web Companion: Taste {} gedrückt", note_val));
    ...
    <script>
        ...
        async function loadCompanion() {{
            try {{
                    ...
                    for (let c = 0; c < 8; c++) {{
                        ...
                        if (mapping) {{
                            ...
                            if (mapping.state) {{
                                ...
                                btn.style.borderColor = "rgb(56, 189, 248)";
                            }}
                        }} else {{
                            btn.innerText = note;
                ...
                grid.innerHTML = "<div style="grid-column: span 8; text-align: center; color: rgb(239, 68, 68);">Ladefehler</div>";
            }}
        }}
